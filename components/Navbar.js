...
const Navbar = ({ logout, user, cart, addToCart, removeFromCart, clearCart, subTotal }) => {
  ...
  return (
    <>
      {!sidebar && <span onMouseOver={() => { setDropdown(true) }} onMouseLeave={() => { setDropdown(false) }} className='fixed right-12 top-4 z-30 cursor-pointer' >
      ...
      <div className={`navbar-show flex flex-col md:flex-row bg-white md:justify-start justify-center items-center py-1 md:py-0 shadow-md top-0 z-10 fixed w-full ${!sidebar && 'overflow-hidden'}`}>
        <div className="logo mx-5 my-1 ">
          <Link href={'/'}><a className="mx-4"><Image src="/logo.png" alt="" width={80} height={50} /></a></Link>
        </div>
        <div className="nav">
          <ul className="flex items-center space-x-7 font-bold text-md ">
            <Link href={'/tshirts'}><a><li className='hover:text-indigo-700 text-black cursor-pointer'>
              T-shirts
            </li></a></Link>
            <Link href={'/hoodies'}><a><li className='hover:text-indigo-700 text-black cursor-pointer'>
              Hoodies
            </li></a></Link>
            <Link href={'/jeans'}><a><li className='hover:text-indigo-700 text-black cursor-pointer'>
              Jeans
            </li></a></Link>
            <Link href={'/trousers'}><a><li className='hover:text-indigo-700 text-black cursor-pointer'>
              Trousers
            </li></a></Link>
          </ul>
        </div>
