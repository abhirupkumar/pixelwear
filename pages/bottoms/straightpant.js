...
const Straightpant = ({ products, titlefilter }) => {
        ...
        <div>
            <section className="text-gray-600 body-font min-h-screen flex flex-row">
                <div className="mx-2 my-8 border border-gray-400 w-1/4 rounded-md h-[80vh]">
                    ...
                    <hr />
                    <p className='flex flex-row justify-center px-1 my-2 mx-10 text-xl'>Subcategory</p>
                    ...
                    <div className='flex flex-col justify-right px-6 mx-auto text-base'>
                        {Object.keys(products).map((item, index) => { return <Link passHref={true} key={index} href={`/product/bottoms?subcategory=${router.query.subcategory}&title=${products[item].title}`}><li className='text-sm cursor-pointer hover:text-blue-600 hover:underline'>{products[item].title}</li></Link> })}
                    </div>
                </div>
...
export async function getServerSideProps(context) {
    ...
    let product = await Product.find({ category: 'bottoms', theme: 'straightpant' })
    let products
    ...
    var index = 0
    for (let item of products) {
        if (!(item.title in titlefilter)) {
            titlefilter[index] = item.title
            index = index + 1
        }
    }
    return {
        props: { products: JSON.parse(JSON.stringify(bottoms)), titlefilter: titlefilter }, // will be passed to the page component as props
        // props: { products: JSON.parse(JSON.stringify(products)) }, // will be passed to the page component as props
    }
}

export default Straightpant