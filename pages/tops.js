...
const Tops = ({ products, filter, colorfilter, page, totalPages }) => {
    ...
    const applyFilters = () => {
        ...
        const filteredUrl = `/tops${queryString}`;
    ...
    useEffect(() => {
        setValue(router.query.category ? router.query.category : 'tops')
    }, [router])
                    ...
                    <div className={`${showFilter ? 'lg:block transition-all overflow-y-scroll' : 'lg:block hidden'}`}>
                        ...
                        <div className={`flex flex-col px-10 mx-auto text-base`}>
                            <FormControl>
                                ...
                                <FormControlLabel
                                    key={1}
                                    control={<Checkbox checked={!!selectedFilters.categories.includes("tshirts")} onChange={handleFilterChange} name="categories" value="tshirts" />}
                                    label="T-shirts"
                                />
                                <FormControlLabel
                                    key={2}
                                    control={<Checkbox checked={!!selectedFilters.categories.includes("hoodies")} onChange={handleFilterChange} name="categories" value="hoodies" />}
                                    label="Hoodies"
                                />
                            </FormControl>
                        </div>
                            ...
                            return <div key={products[item]._id} className="lg:w-[310px] w-[39%] cursor-pointer m-4 overlfow-x-hiden">
                                <Link href={`/product/${products[item].slug}`}>
                                    <div className="flex justify-center md:h-[480px] h-[216px] overflow-hidden">
                                        <img alt="ecommerce" className="m-auto md:m-0" src={products[item].img} loading="lazy" />
                                    </div>
                                    <div className="text-center md:text-left flex flex-col lg:h-[195px] h-[162px] justify-around">
                                        ...
                                        <div className="flex space-x-2 items-center">
                                            ...
                                            {!!products[item].mrp && discount > 0 && <p className="mt-1 text-left font-semibold text-red-600 text-sm">({discount}% off)</p>}
                                        </div>
