...
const Jeggings = ({ products }) => {
    return (
        <div>
            <section className="text-gray-600 body-font min-h-screen">
                <div className="container px-5 py-24 mx-auto">
                    <div className="flex flex-wrap -m-4 justify-center">
                    {Object.keys(products).length === 0 && <p>Sorry all the jeggings are currently out of stock. New stock comming soon. Stay Tuned!</p>}
...
export async function getServerSideProps(context) {
    ...
    let products = await Product.find({ category: 'jeggings' })
    let jeggings = {}
    for (let item of products) {
        if (item.title in jeggings) {
            if (!jeggings[item.title].color.includes(item.color) && item.availableQty > 0) {
                jeggings[item.title].color.push(item.color)
            }
            if (!jeggings[item.title].size.includes(item.size) && item.availableQty > 0) {
                jeggings[item.title].size.push(item.size)
            }
        }
        else {
            jeggings[item.title] = JSON.parse(JSON.stringify(item))
            if (item.availableQty > 0) {
                jeggings[item.title].color = [item.color]
                jeggings[item.title].size = [item.size]
            }
            else{
                jeggings[item.title].color = []
                jeggings[item.title].size = []
            }
        }
    }
    return {
        props: { products: JSON.parse(JSON.stringify(jeggings)) }, // will be passed to the page component as props
